...
- [Django Intro](#Intro)
- [Django 구조 이해하기 (MTV Design Pattern)](#Django-구조-이해하기)
...
- #### MVC 소프트웨어 디자인 패턴
  ...
  1. Controller : 명령을 model과 view 부분으로 연결
...
- #### 정리 :heavy_check_mark:
  ...
  1. `$ mkdir 00_django_intro` 폴더 생성

  1. `$ cd 00_django_intro` 폴더로 이동

  1. `$ python -m venv [venv]` 가상환경 생성

  1. `$ source [venv]/scripts/activate` 가상환경 접속

  1. `$ pip install django==3.2.15` 로 설치

  1. `$ pip freeze > requirements.txt` 

  1. `$ django-admin startproject [firstpjt] .` 프로젝트 생성

  1. `$ python manage.py startapp articles`

  1. settings.py -> INSTALLED_APPS 안에 `'articles',` 추가

  1. `$ python manage.py runserver` 프로젝트 서버 실행
